...
<main class="mx-auto max-w-7xl px-5 py-10 md:px-8">
    ...
    <article class="surface-panel rounded-card p-6">
      ...
      >
        {#if analyticsDisabled}
          <ToggleRight size={18} />
          {t('Anonymous analytics disabled')}
        {:else}
          <ToggleLeft size={18} />
          {t('Disable anonymous analytics')}
        {/if}
      </button>
    </article>
  </section>
  ...
  <section class="mt-8 grid gap-5 md:grid-cols-2">
    <article class="surface-panel rounded-card p-6">
      ...
      <h2 class="text-2xl font-semibold text-on-surface">{t('What stays in local storage')}</h2>
      <div class="mt-5 grid gap-4">
        {#each localStorageItems as item}
          <div class="rounded-lg border border-outline-variant bg-surface-low p-5">
            <h3 class="text-lg font-semibold text-on-surface">{t(item.title)}</h3>
            <p class="mt-2 text-sm leading-6 text-on-surface-variant">{t(item.body)}</p>
          </div>
        {/each}
      </div>
    </article>
    ...
    <article class="surface-panel rounded-card p-6">
      ...
      <h2 class="text-2xl font-semibold text-on-surface">{t('What is not collected')}</h2>
      <div class="mt-5 grid gap-3 text-sm leading-6 text-on-surface-variant">
        <p class="rounded-lg border border-outline-variant bg-surface-low p-4">{t('No private keys, seed phrases, wallet connections, or exchange credentials are requested.')}</p>
        <p class="rounded-lg border border-outline-variant bg-surface-low p-4">{t('QR payloads, pasted public addresses, amounts, labels, and custom logos are not uploaded to an address vault.')}</p>
        <p class="rounded-lg border border-outline-variant bg-surface-low p-4">{t('Saved data can be removed by deleting items in the app or clearing this site browser storage.')}</p>
      </div>
    </article>
  </section>
